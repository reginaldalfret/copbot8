...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog-components"
...
export function ChatInterface() {
  ...
  const guides: StepByStepGuide[] = loadGuides()
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      ...
      toast({
        title: t('toast.onlineMode'),
        description: t('toast.onlineDescription'),
      })
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "system",
          content: t('error.processingError'),
        },
      ])
    } finally {
      setIsLoading(false)
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setKnowledgeBase(data)
        toast({
          title: t('toast.knowledgeBaseUpdated'),
          description: t('toast.knowledgeBaseDescription'),
        })
      } catch (error) {
        toast({
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        setTimeout(() => {
          ...
          const simulatedText = t('simulatedVoice.exampleQuery')
          setInput(simulatedText)
          setIsLoading(false)

          toast({
            title: t('toast.voiceRecognized'),
            description: t('toast.voiceRecognizedDescription').replace('{text}', simulatedText),
          })
        }, 1500)
      ...
      toast({
        ...
        description: t('toast.recordingDescription'),
      })
    } catch (error) {
      console.error("Error accessing microphone:", error)
      toast({
        title: t('toast.microphoneError'),
        description: t('toast.microphoneErrorDescription'),
        variant: "destructive",
      })
    }
  }
    ...
    <div className="flex flex-col h-[calc(100vh-30rem)]">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <Select value={language} onValueChange={(value) => {
            // Language is handled through the useLanguage hook
              setLanguage(value)
            }
          }}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="chat" className="flex-1 flex flex-col">
          ...
          <div className="bg-muted/50 p-3 rounded-lg mb-4">
            <h3 className="text-sm font-medium mb-2">{t('chat.quickQuestions')}</h3>
      ...
      <Dialog open={showEmergencyDialog} onOpenChange={setShowEmergencyDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-emergency-600">{t('emergency.dialogTitle')}</DialogTitle>
            <DialogDescription>{t('emergency.dialogDescription')}</DialogDescription>
          </DialogHeader>
